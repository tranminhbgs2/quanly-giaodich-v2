...
class Htmldomnode
{
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key=>$val)
		{
			...
			$ret .= $this->_[YANGQI_HDOM_INFO_SPACE][$i][0];
			//no value attr: nowrap, checked selected...
			if ($val===true)
				$ret .= $key;
			else {
				switch ($this->_[YANGQI_HDOM_INFO_QUOTE][$i])
				{
					case YANGQI_HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case YANGQI_HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}
				$ret .= $key.$this->_[YANGQI_HDOM_INFO_SPACE][$i][1].'='.$this->_[YANGQI_HDOM_INFO_SPACE][$i][2].$quote.$val.$quote;
			}
		}
...
//		$pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
	...
	function __set($name, $value)
	{
		...
		if (!isset($this->attr[$name]))
		{
			$this->_[YANGQI_HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[YANGQI_HDOM_INFO_QUOTE][] = YANGQI_HDOM_QUOTE_DOUBLE;
		}
